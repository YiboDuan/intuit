...
var diffObjectChanges = function (oldItem, newItem, preventNestedDiff) {                                             // 137
  var result = {};                                                                                                   // 138
                                                                                                                     // 139
  angular.forEach(newItem, function (value, key) {                                                                   // 140
    var oldValue;                                                                                                    // 141
                                                                                                                     // 142
    if (oldItem && typeof oldItem === 'object')                                                                      // 143
      oldValue = oldItem[key];                                                                                       // 144
                                                                                                                     // 145
    if (oldItem && angular.equals(value, oldValue))                                                                  // 146
      return;                                                                                                        // 147
                                                                                                                     // 148
    if (key == '$$hashKey')                                                                                          // 149
      return;                                                                                                        // 150
                                                                                                                     // 151
    if (isActualObject(value)) {                                                                                     // 152
                                                                                                                     // 153
      if (_.isEmpty(value)) {                                                                                        // 154
        result[key] = value;                                                                                         // 155
      }                                                                                                              // 156
      else if (!preventNestedDiff) {                                                                                 // 157
        var diff = diffObjectChanges(oldValue, value);                                                               // 158
        if (diff) result[key] = diff;                                                                                // 159
      }                                                                                                              // 160
                                                                                                                     // 161
    } else {                                                                                                         // 162
      result[key] = value;                                                                                           // 163
    }                                                                                                                // 164
                                                                                                                     // 165
    // If a nested object is identical between newItem and oldItem, it                                               // 166
    // is initially attached as an empty object. Here we remove it from                                              // 167
    // the result if it was not empty from the beginning.                                                            // 168
    if (isActualObject(result[key]) && _.keys(result[key]).length === 0) {                                           // 169
      if (_.keys(value).length !== 0)                                                                                // 170
        delete result[key];                                                                                          // 171
    }                                                                                                                // 172
  });                                                                                                                // 173
                                                                                                                     // 174
  var resultKeys = _.keys(result);                                                                                   // 175
  var emptyResult = resultKeys.length === 0;                                                                         // 176
  var hashKeyOnly = resultKeys.length === 1 && !!result.$$hashKey;                                                   // 177
                                                                                                                     // 178
  if (emptyResult || hashKeyOnly)                                                                                    // 179
    return undefined;                                                                                                // 180
                                                                                                                     // 181
  return flattenObject(result);                                                                                      // 182
};                                                                                                                   // 183
                                                                                                                     // 184
// Diffs two objects and returns the keys that have been removed.                                                    // 185
// Can be used to construct a Mongo {$unset: {}} modifier                                                            // 186
var diffObjectRemovals = function (oldItem, newItem, preventNestedDiff) {                                            // 187
  var oldItemKeys = _.keys(oldItem);                                                                                 // 188
  var newItemKeys = _.keys(newItem);                                                                                 // 189
                                                                                                                     // 190
  var result = {};                                                                                                   // 191
                                                                                                                     // 192
  angular.forEach(oldItemKeys, function (key) {                                                                      // 193
                                                                                                                     // 194
    // Check if property was removed.                                                                                // 195
    if (!_.contains(newItemKeys, key) ||                                                                             // 196
      angular.isUndefined(newItem[key])) {                                                                           // 197
      if (key != '$$hashKey')                                                                                        // 198
        result[key] = true;                                                                                          // 199
    }                                                                                                                // 200
                                                                                                                     // 201
    // A nulled field is not considered a removal.                                                                   // 202
    else if (_.isNull(newItem[key])) {                                                                               // 203
      return;                                                                                                        // 204
    }                                                                                                                // 205
                                                                                                                     // 206
    // Perform nested diff.                                                                                          // 207
    else if (isActualObject(oldItem[key]) && !preventNestedDiff) {                                                   // 208
      if (key != '$$hashKey') {                                                                                      // 209
        var diff = diffObjectRemovals(oldItem[key], newItem[key]);                                                   // 210
        if (diff) result[key] = diff;                                                                                // 211
      }                                                                                                              // 212
    }                                                                                                                // 213
  });                                                                                                                // 214
                                                                                                                     // 215
  if (_.keys(result).length === 0)                                                                                   // 216
    return undefined;                                                                                                // 217
                                                                                                                     // 218
  return flattenObject(result);                                                                                      // 219
};                                                                                                                   // 220
                                                                                                                     // 221
var handleDeepProperties = function(object, paths, cb) {                                                             // 222
  angular.forEach(paths, function(value, path) {                                                                     // 223
    var props = path.split('.');                                                                                     // 224
    var currentObject = object;                                                                                      // 225
                                                                                                                     // 226
    for (var i = 0; i < props.length - 1; i++) {                                                                     // 227
      var tmp = currentObject[props[i]];                                                                             // 228
      if (!_.isObject(tmp) || _.isArray(tmp))                                                                        // 229
        currentObject[props[i]] = {};                                                                                // 230
      currentObject = currentObject[props[i]];                                                                       // 231
    }                                                                                                                // 232
    cb(currentObject, props[i], value);                                                                              // 233
  });                                                                                                                // 234
};                                                                                                                   // 235
                                                                                                                     // 236
var deepCopyObjectChanges = function (oldItem, newItem) {                                                            // 237
  var setDiff = diffObjectChanges(oldItem, newItem);                                                                 // 238
  handleDeepProperties(oldItem, setDiff, function setter(object, prop, value) {                                      // 239
    object[prop] = value;                                                                                            // 240
  });                                                                                                                // 241
};                                                                                                                   // 242
                                                                                                                     // 243
var deepCopyObjectRemovals = function (oldItem, newItem) {                                                           // 244
  var unsetDiff = diffObjectRemovals(oldItem, newItem);                                                              // 245
  handleDeepProperties(oldItem, unsetDiff, function deleter(object, prop) {                                          // 246
    delete object[prop];                                                                                             // 247
  });                                                                                                                // 248
};                                                                                                                   // 249
                                                                                                                     // 250
module.value('diffArray', diffArray);                                                                                // 251
module.value('deepCopyChanges', deepCopyObjectChanges);                                                              // 252
module.value('deepCopyRemovals', deepCopyObjectRemovals);                                                            // 253
                                                                                                                     // 254
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
angularMeteorObject.factory('AngularMeteorObject', ['$q', '$meteorSubscribe', function($q, $meteorSubscribe) {       // 3
  ...
  AngularMeteorObject.save = function save(docs) {                                                                   // 17
    ...
    return deferred.promise;                                                                                         // 39
                                                                                                                     ...
                                                                                                                     // 41
  AngularMeteorObject.reset = function reset(keepClientProps) {                                                      // 42
    var self = this,                                                                                                 // 43
    ...
    if (collection){                                                                                                 // 48
      ...
      var props;                                                                                                     // 51
      if (serverValue) {                                                                                             // 52
        angular.extend(Object.getPrototypeOf(self), Object.getPrototypeOf(serverValue));                             // 53
        for (prop in serverValue) {                                                                                  // 54
          if (serverValue.hasOwnProperty(prop)) {                                                                    // 55
            self[prop] = serverValue[prop];                                                                          // 56
            self._serverBackup[prop] = serverValue[prop];                                                            // 57
          }                                                                                                          // 58
        }                                                                                                            // 59
                                                                                                                     // 60
        if (keepClientProps) {                                                                                       // 61
          props = _.intersection(_.keys(self), _.keys(self._serverBackup));                                          // 62
        } else {                                                                                                     // 63
          props = _.keys(self);                                                                                      // 64
        }                                                                                                            // 65
        var serverProps = _.keys(serverValue);                                                                       // 66
        var removedKeys = _.difference(props, serverProps, self.$$internalProps);                                    // 67
        _.each(removedKeys, function (prop) {                                                                        // 68
          delete self[prop];                                                                                         // 69
          delete self._serverBackup[prop]                                                                            // 70
        });                                                                                                          // 71
      } else {                                                                                                       // 72
        for (prop in _.omit(self, self.$$internalProps)) {                                                           // 73
          delete self[prop];                                                                                         // 74
        }                                                                                                            // 75
        self._serverBackup = {};                                                                                     // 76
      }                                                                                                              // 77
    }                                                                                                                // 78
  };                                                                                                                 // 79
                                                                                                                     // 80
  AngularMeteorObject.stop = function stop() {                                                                       // 81
    if (this.unregisterAutoDestroy) {                                                                                // 82
      this.unregisterAutoDestroy();                                                                                  // 83
    }                                                                                                                // 84
    this.unregisterAutoDestroy = null;                                                                               // 85
                                                                                                                     // 86
    if (this.unregisterAutoBind) {                                                                                   // 87
      this.unregisterAutoBind();                                                                                     // 88
    }                                                                                                                // 89
    this.unregisterAutoBind = null;                                                                                  // 90
                                                                                                                     // 91
    if (this.autorunComputation && this.autorunComputation.stop) {                                                   // 92
      this.autorunComputation.stop();                                                                                // 93
    }                                                                                                                // 94
    this.autorunComputation = null;                                                                                  // 95
  };                                                                                                                 // 96
                                                                                                                     // 97
// A list of internals properties to not watch for, nor pass to the Document on update and etc.                      // 98
  AngularMeteorObject.$$internalProps = [                                                                            // 99
    ...
    'collection', '_eventEmitter', '_serverBackup'                                                                   // 101
  ];                                                                                                                 // 102
                                                                                                                     // 103
  var createAngularMeteorObject = function(collection, id, options){                                                 // 104
    // Make data not be an object so we can extend it to preserve                                                    // 105
    // Collection Helpers and the like                                                                               // 106
    var data = new function SubObject() {};                                                                          // 107
    var doc = collection.findOne(id, options)                                                                        // 108
    angular.extend(data, doc);                                                                                       // 109
                                                                                                                     // 110
    data._serverBackup = doc || {};                                                                                  // 111
    data.$$collection = collection;                                                                                  // 112
    data.$$options = options;                                                                                        // 113
    data.$$id = id;                                                                                                  // 114
                                                                                                                     // 115
    angular.extend(data, AngularMeteorObject);                                                                       // 116
                                                                                                                     // 117
    return data;                                                                                                     // 118
  };                                                                                                                 // 119
                                                                                                                     // 120
  return createAngularMeteorObject;                                                                                  // 121
}]);                                                                                                                 // 122
                                                                                                                     // 123
                                                                                                                     // 124
angularMeteorObject.factory('$meteorObject', ['$rootScope', '$meteorUtils', 'AngularMeteorObject',                   // 125
  function($rootScope, $meteorUtils, AngularMeteorObject) {                                                          // 126
    return function(collection, id, auto, options) {                                                                 // 127
      // Validate parameters                                                                                         // 128
      if (!collection) {                                                                                             // 129
        throw new TypeError("The first argument of $meteorObject is undefined.");                                    // 130
      }                                                                                                              // 131
      if (!angular.isFunction(collection.findOne)) {                                                                 // 132
        throw new TypeError("The first argument of $meteorObject must be a function or a have a findOne function property.");
      }                                                                                                              // 134
                                                                                                                     // 135
      auto = auto !== false; // Making auto default true - http://stackoverflow.com/a/15464208/1426570               // 136
                                                                                                                     // 137
      var data = new AngularMeteorObject(collection, id, options);                                                   // 138
                                                                                                                     // 139
      data.autorunComputation = $meteorUtils.autorun($rootScope, function() {                                        // 140
        data.reset(true);                                                                                            // 141
      });                                                                                                            // 142
                                                                                                                     // 143
      if (auto) { // Deep watches the model and performs autobind.                                                   // 144
        data.unregisterAutoBind = $rootScope.$watch(function(){                                                      // 145
          return _.omit(data, data.$$internalProps);                                                                 // 146
        }, function (newItem, oldItem) {                                                                             // 147
          if (newItem !== oldItem && newItem) {                                                                      // 148
            var newItemId = newItem._id;                                                                             // 149
            if (newItemId && !_.isEmpty(newItem = _.omit(angular.copy(newItem), '_id'))) {                           // 150
              collection.update({_id: newItemId}, {$set: newItem});                                                  // 151
            }                                                                                                        // 152
          }                                                                                                          // 153
        }, true);                                                                                                    // 154
      }                                                                                                              // 155
                                                                                                                     // 156
      data.unregisterAutoDestroy = $rootScope.$on('$destroy', function() {                                           // 157
        if (data && data.stop) {                                                                                     // 158
          data.stop();                                                                                               // 159
        }                                                                                                            // 160
        data = undefined;                                                                                            // 161
      });                                                                                                            // 162
      ...
      return data;                                                                                                   // 164
                                                                                                                     ...
                                                                                                                     // 167
angularMeteorObject.run(['$rootScope', '$q', '$meteorObject', '$meteorSubscribe',                                    // 168
  function($rootScope, $q, $meteorObject, $meteorSubscribe) {                                                        // 169
    Object.getPrototypeOf($rootScope).$meteorObject = function() {                                                   // 170
      var args = Array.prototype.slice.call(arguments);                                                              // 171
      var object = $meteorObject.apply(this, args);                                                                  // 172
      var subscription = null;                                                                                       // 173
                                                                                                                     // 174
      object.subscribe = function () {                                                                               // 175
        var args = Array.prototype.slice.call(arguments);                                                            // 176
        subscription = $meteorSubscribe._subscribe(this, $q.defer(), args);                                          // 177
        return object;                                                                                               // 178
      };                                                                                                             // 179
                                                                                                                     // 180
      this.$on('$destroy', function() {                                                                              // 181
        object.stop();                                                                                               // 182
        if (subscription)                                                                                            // 183
          subscription.stop();                                                                                       // 184
	  });                                                                                                               // 185
                                                                                                                     // 186
      return object;                                                                                                 // 187
	};                                                                                                                  // 188
